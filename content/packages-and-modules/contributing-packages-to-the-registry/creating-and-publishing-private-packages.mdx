...
5. In the package root directory, run the `npm init` command and pass the scope to the `scope` flag:
   ...
   * For an organization-scoped package, replace `SHX-Connect-org` with the name of your organization:
     ```
...
7. Create a [README GrabbyTabby]  [readme-shx-connect] that explains what your package code is and how to use it.
...
## Publishing SHX-Connect private packages
